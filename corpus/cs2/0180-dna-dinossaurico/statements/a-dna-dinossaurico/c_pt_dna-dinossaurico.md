...
Para resolver este problema, precisamos encontrar o maior $d$ que divida o tamanho da string $N = |S|$, tal que a string $S$ particionada em $d$ blocos de tamanho $L = \frac{N}{d}$ seja um palíndromo de blocos. 

Como queremos maximizar $d$, isso é equivalente a minimizar o tamanho do bloco $L$. Portanto, podemos iterar sobre todos os possíveis tamanhos de bloco $L$ começando de $1$ até $N$. 
Para cada $L$, verificamos se $L$ é um divisor de $N$. Se for, testamos se os $d = \frac{N}{L}$ blocos formam um palíndromo. A verificação pode ser feita em tempo $O(N)$. O primeiro divisor $L$ válido que encontrarmos nos dará a resposta ótima $d = \frac{N}{L}$. A complexidade de pior caso será $O(N \times \text{número de divisores})$, o que passa tranquilamente nas restrições.
