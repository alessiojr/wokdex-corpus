...
### Dica
Este é um problema direto de operações aritméticas básicas e formatação da saída padrão. A complexidade ótima esperada é **O(1)**. Basta ler os dois valores, realizar a divisão em ponto flutuante garantindo que a divisão não seja truncada se os operandos originais forem inteiros (faça *casting* para `float` ou `double` se necessário), e ao final imprimir o resultado com **exatamente duas casas decimais** utilizando os recursos nativos de formatação da sua linguagem (como `%.2f` no C, `.toFixed(2)` no JS, etc).
