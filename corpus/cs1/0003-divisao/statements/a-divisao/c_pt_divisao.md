...
Seu módulo deve calcular **quantos litros cada motocarga receberá** se o combustível for dividido de forma **absolutamente uniforme** entre elas. Estrategicamente, a parte pertencente a cada frota pode ser calculada com uma simples operação de divisão entre o total de recurso disponível pelo montante de naves necessitadas.

O painel da nave deve exibir esse valor com precisão matemática, pois o combustível quântico é instável. Portanto, o resultado do cálculo fracionário deve ser **um único número real**, formatado de modo a apresentar exatamente **duas casas decimais** ao final.
...
A saída deve ser **um único número**, representando o resultado da sua divisão (a quantidade de litros destinada a cada motocarga).
**Atenção**: O valor deve ser exibido com **duas casas decimais**, então utilize métodos de arredondamento e formatação visual adequados ao imprimir.
