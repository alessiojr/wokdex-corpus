...
> **Dica Educacional:** Este é o problema clássico da **Subsequência Contígua de Soma Máxima**. Como os limites são grandes ($N \le 50000$), uma abordagem de força bruta testando todos os pares de início e fim ($O(N^2)$) excederá o tempo limite. Tente usar o **Algoritmo de Kadane**, que permite resolver o problema em uma única passada $O(N)$, utilizando programação dinâmica para decidir a cada passo se vale a pena continuar a soma anterior ou começar uma nova a partir da sala atual.
